// Replace Infura with Alchemy
const alchemyApiKey = "wYd5CZx_OfJQ5uzgFOy4BxvDs5ZXtchA"; // Replace with your Alchemy API Key
const alchemyIPFSUrl = `https://ipfs.alchemy.com/v2/${alchemyApiKey}/add`;

window.CONTRACT = {
  address: "0x4946a9eb5d83B2b526f45cfC21FC70Ca78DDFB29",
  network: "https://eth-sepolia.g.alchemy.com/v2/wYd5CZx_OfJQ5uzgFOy4BxvDs5ZXtchA",
  explore: "https://etherscan.com/",
  abi: [
    {
      inputs: [],
...
async function uploadFileToIpfs() {
  ...
  try {
    const response = await fetch(alchemyIPFSUrl, {
      method: "POST",
      body: formData,
    });
    ...
    console.log("Uploaded file CID:", data.path);
    return data.path; // CID returned from Alchemy
  } catch (error) {
    console.error("Error uploading file to Alchemy IPFS:", error);
    throw error;
  }
}
...
async function sendHash() {
  ...
  // 🛠️ Check Web3 & MetaMask Connection
  if (typeof window.ethereum === "undefined") {
    console.log("MetaMask not detected!");
    $("#note").html(`<h5 class="text-danger">MetaMask not installed! 🛑</h5>`);
    $("#loader").addClass("d-none");
    $("#upload_file_button").slideDown();
    return;
  }

  // 🛠️ Ensure User Wallet is Connected
  if (!window.userAddress) {
    console.log("User wallet not connected!");
    $("#note").html(`<h5 class="text-danger">Wallet not connected! 🛑</h5>`);
    $("#loader").addClass("d-none");
    $("#upload_file_button").slideDown();
    return;
  }

  // 🛠️ Ensure Contract is Initialized
  if (!window.contract) {
    console.log("Contract not initialized!");
    $("#note").html(`<h5 class="text-danger">Smart contract not initialized! 🛑</h5>`);
    $("#loader").addClass("d-none");
    $("#upload_file_button").slideDown();
    return;
  }

  // 🛠️ Ensure MetaMask is Open
  await window.ethereum.request({ method: "eth_requestAccounts" });

  const CID = await uploadFileToIpfs();
  if (window.hashedfile.length > 4) {
    console.log("Hashed file:", window.hashedfile);
    console.log("CID:", CID);
...
async function get_ChainID() {
  ...
  switch (a) {
    ...
    case 42:
      ...
      break;
    case 11155111:  // ✅ Added Sepolia Testnet
